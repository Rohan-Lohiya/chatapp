...
const page = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (isClickOutsideMenu && isClickOutsideImage && isClickOutsideClearChat && isClickOutsideDeleteChat && isClickOutsideAddChat) {
...
const { chatData, isLoading, error } = useSocketAndChatData(session);
  useEffect(() => {
    if (session === null) {
      redirect('/login');
    }
        ...
        <div className={styles.secondheading}>
          ...
          {firstdivselected === '1' && <span className={styles.imagecont}>
            <span className={styles.addchatimgcont}>
              <Image onClick={() => setaddchatpressed(true)} className={styles.optImage} src={'/addchat.png'} width={25} height={25} alt='AddChat'></Image>
              {addchatpressed && <span ref={addchatref} className={styles.addchatcont}><AddChat/></span>}
            </span>
        ...
        {isselectedGID && <>
           <div className={styles.thirdheading}>
             <span className={styles.selectedchatinfocont}>
               <Image className={styles.selectedavtarimage} src={selectedavtar(selectedGoogleID)} width={40} height={40} alt='Group'></Image>
           ...
           <div className={styles.selectedchatcont}><SelectedChats GoogleID = {selectedGoogleID}/></div>
